class Solution:
    def productExceptSelf(self, nums: List[int]) -> List[int]:
        ...
        # if the number of zeros is > 1, then the total product is 0 because you only disclude one number at a time
        # e.i [0,8,0] you can only disclude 1 zero but multiplying the other two results in a 0 product.
        num_zeros = 0
        for i in nums:
            if i  == 0:
                num_zeros += 1
